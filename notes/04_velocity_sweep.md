...
2. **Flame migrates toward the geometric midpoint as $U_{jet}$ rises.** At $U_{jet} = 0.05$ m/s the peak temperature sits at $x = 12.6$ mm — well on the air side. Each step of $U_{jet}$ pushes the flame about 0.5–1.0 mm toward $x = 10$ mm; at $U_{jet} = 0.25$ m/s the flame *exactly* hits the geometric midpoint, and slightly overshoots to 9.8 mm at $U_{jet} = 0.30$ m/s. As strain rate rises, residence time in the mixing layer drops and the finite-rate chemistry has less margin to shift the peak heat-release location away from the stoichiometric mixture-fraction surface — so the flame migrates toward the location where the diffusive fluxes from the two boundaries balance, which for symmetric momentum sits near the geometric midpoint.
3. **FWHM shrinks monotonically from 7.8 mm to 2.8 mm.** Higher jet velocity = higher strain rate = thinner diffusive heating zone. Quantitatively, FWHM × $\sqrt{U_{jet}}$ ≈ 1.5 mm·s$^{0.5}$ across the sweep (range 1.48–1.74, within 18 %), consistent with the diffusion-flame thickness scaling $\delta \sim (D / \dot{\gamma})^{0.5}$ where the strain rate $\dot{\gamma}$ scales linearly with $U_{jet}$.
